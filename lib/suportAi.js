...
export const formatNaturalText = (data, action) => {
  ...
  // 📊 TẠO DỮ LIỆU CHO BIỂU ĐỒ (Dành cho Chart.js / Recharts trên Frontend)
  const charts = [
    // 1. Biểu đồ tròn (Pie Chart) - Tỷ trọng phương thức thanh toán
    {
      type: "pie",
      title: "Cơ cấu doanh thu theo thanh toán",
      dataKey: "total", // Khóa lấy giá trị
      nameKey: "methodLabel", // Khóa lấy tên hiển thị
      data: methods.map((m) => ({
        methodLabel: methodLabel[m.method] || m.method,
        total: Number(m.total || 0),
      })),
    },
    // 2. Biểu đồ cột (Bar Chart) - KPI tổng quan
    {
      type: "bar",
      title: "Tổng quan KPI",
      dataKey: "value",
  ...
  return {
    ...
    charts: charts, // <--- Trả thêm data chart về FE
  };
};
...
export const formatRoomTablePayload = (roomNumber, action, payload) => {
  ...
  // 📊 TẠO DỮ LIỆU CHO BIỂU ĐỒ PHÒNG CỤ THỂ
  const charts = [
    {
      type: "pie",
      title: "Thanh toán theo hình thức",
